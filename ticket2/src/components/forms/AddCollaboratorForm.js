...
import { addNewCollaborator } from "@/store/slices/eventCollaboratorSlice";
...
const AddCollaboratorForm = ({ setOpenModal, eventId }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        dispatch(addNewCollaborator(response));
        onCloseModal();
      } else {
  ...
  return (
    <>
      <h3 className=" mb-6 mt-4 text-xl font-medium text-gray-900 dark:text-white transition-colors hover:bg-purple-700 bg-purple-600">
